...
async def reader_lw_test(dut):
    ...
    for _ in range(100):
        ...
        await Timer(1, units="ns")
        assert dut.write_back_data.value == mem_data
...
async def reader_lh_test(dut):
    ...
    for _ in range(100):
        ...
        dut.mem_data.value = mem_data
        await Timer(1, units="ns")
        assert dut.write_back_data.value == (mem_data & 0xFFFF0000) >> 16
        assert dut.valid.value == 1
        ...
        await Timer(1, units="ns")
        assert int(dut.write_back_data.value) - (1 << 32) == expected
        assert dut.valid.value == 1

    dut.byte_enable.value = 0b0011
    await Timer(1, units="ns")
    for _ in range(100):
        # UNSIGNED
        ...
        dut.mem_data.value = mem_data
        await Timer(1, units="ns")
        assert dut.write_back_data.value == (mem_data & 0x0000FFFF)
        assert dut.valid.value == 1
...
async def reader_lb_test(dut):
    ...
    for _ in range(100):
        ...
        dut.mem_data.value = mem_data
        await Timer(1, units="ns")
        assert dut.write_back_data.value == (mem_data & 0xFF000000) >> 24
        assert dut.valid.value == 1
    ...
    for _ in range(100):
        ...
        dut.mem_data.value = mem_data
        await Timer(1, units="ns")
        assert dut.write_back_data.value == (mem_data & 0x00FF0000) >> 16
        assert dut.valid.value == 1
    ...
    for _ in range(100):
        ...
        dut.mem_data.value = mem_data
        await Timer(1, units="ns")
        assert dut.write_back_data.value == (mem_data & 0x0000FF00) >> 8
        assert dut.valid.value == 1
    ...
    for _ in range(100):
        ...
        dut.mem_data.value = mem_data
        await Timer(1, units="ns")
        assert dut.write_back_data.value == (mem_data & 0x000000FF)
        assert dut.valid.value == 1
